...
function main() {
  ...
  const generateScript = path.join(scriptDir, "generate-product-image.mjs");
  ...
  if (args.productImageUrl) {
    command.push("--product-image-url", args.productImageUrl);
  }

  const result = spawnSync(process.execPath, command, { stdio: "inherit" });
  if (result.status !== 0) fail("Product image generation failed.");
  ...
  console.log(`Product bundle written to ${outDir}`);
}
